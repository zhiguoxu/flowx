...
from auto_flow.core.rag.document.document import Document, MetadataMapping
...
def default_suffix_file_loaders() -> Dict[str, Type[FileLoader]]:
    from auto_flow.core.rag.document_loaders.file_loaders.pdf_loader import PDFLoader
    from auto_flow.core.rag.document_loaders.file_loaders.docx_loader import DocxLoader
    return dict(
        pdf=PDFLoader,
        docx=DocxLoader
    )
